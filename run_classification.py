...
Examples:
  ...
  python run_classification.py --mode single --samples-per-class 10 --epochs 3
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="ashhadahsan/ai-vs-human-music-with-audio",
    ...
    try:
        if args.mode == "single":
            ...
            if "classification_report" in eval_results:
                ...
                print(f"\nDetailed Metrics:")
                print(
                    f"  Precision: {report.get('weighted avg', {}).get('precision', 0):.4f}"
                )
                print(
                    f"  Recall: {report.get('weighted avg', {}).get('recall', 0):.4f}"
                )
                print(
                    f"  F1 Score: {report.get('weighted avg', {}).get('f1-score', 0):.4f}"
                )
