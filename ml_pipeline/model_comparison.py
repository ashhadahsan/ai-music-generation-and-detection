...
class ModelComparator:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ModelComparator with {len(self.model_configs)} configurations"
        )

    def run_single_experiment(
        self, config: Dict, epochs: int = 5, batch_size: int = 2
    ) -> Dict:
        """
        ...
        try:
            ...
            evaluation_results = pipeline.evaluate_model(
                training_results["trainer"], dataset_dict
            )
        ...
        if successful_results:
            ...
            for result in successful_results:
                ...
                performance = {
                    ...
                    "f1": eval_results.get("classification_report", {})
                    .get("weighted avg", {})
                    .get("f1-score", 0),
                    "precision": eval_results.get("classification_report", {})
                    .get("weighted avg", {})
                    .get("precision", 0),
                    "recall": eval_results.get("classification_report", {})
                    .get("weighted avg", {})
                    .get("recall", 0),
                }
    ...
    def save_comparison_results(self, results: List[Dict], summary: Dict) -> None:
        """
    ...
    def create_comparison_report(self, summary: Dict) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                "| Model | Description | Accuracy | F1 | Precision | Recall | ROC AUC |\n"
            )
            f.write(
                "|-------|-------------|----------|----|-----------|--------|----------|\n"
            )
            ...
            for model in summary["model_performance"]:
                ...
                f.write(
                    f"| {model['model_name']} | {model['description']} | "
    ...
    def create_comparison_plots(self, results: List[Dict]) -> None:
        ...
        for result in successful_results:
            ...
            f1 = (
                eval_results.get("classification_report", {})
                .get("weighted avg", {})
                .get("f1-score", 0)
            )
            f1_scores.append(f1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare different Wav2Vec2 configurations"
    )

    parser.add_argument(
        "--dataset",
        type=str,
        default="ashhadahsan/ai-vs-human-music-dataset",
        help="HuggingFace dataset name",
    )
    parser.add_argument(
        "--samples-per-class", type=int, default=25, help="Number of samples per class"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--epochs", type=int, default=5, help="Number of training epochs for comparison"
    )
    ...
    results = comparator.run_comparison(
        epochs=args.epochs, batch_size=args.batch_size, selected_configs=args.models
    )
